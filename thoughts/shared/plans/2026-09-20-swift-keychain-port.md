...
Interactive macOS Zsh users keep `zenv set`, `ls`, `rm`, `doctor`, and `migrate`. Secrets move from the Door 3 auto-sourced env file into the login keychain. Load happens only in `~/.zshrc` through `eval "$(zenv env)"`. Linux and Door 3 go away. The stack is `swift-store`, `swift-hook`, `swift-cli`, then `swift-retire`. The operator lands the chain.
...
- [ ] Lane 1. Regression lane against trunk. Run `zenv version` on trunk Go and `swift run zenv --help` on head. Trunk lacks Keychain. Gate that head help lists `env` and `keys` and that trunk still writes the Door 3 auto-sourced env file if `zenv set` is used. Save `store-l1-trunk-help.png`. Pass when head help includes `env` and trunk help does not.
...
- [ ] Lane 10. Confirm `$HOME/.zshenv` is not created or rewritten. Save `store-l10-no-zshenv.png`. Pass when the file mtime is unchanged or the file is absent.

**Verify, perf.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked.

- [ ] Metric. Wall time of `zenv env` for 20 keys on head. Wall time of sourcing a 20-line `$HOME/.zshenv` on trunk. Also isolate head process spawn plus `SecItemCopyMatching`.
...
- [ ] Add `doctor` that requires Zsh, requires `~/.zshrc`, and installs a marker block `# --- zenv safe display start ---` whose body runs `eval "$(command zenv env)"`, sets `_ZENV_KEYS` from `command zenv keys` joined by `|`, and keeps `_zenv_masker` plus `env`/`printenv` overrides from `internal/shell/zshrc.go` without sourcing the Door 3 auto-sourced env file. Idempotent install. Backup `~/.zshrc` under `~/.zenv/backups/` as today. If the Door 3 auto-sourced env file contains `export` lines zenv used to own, prompt to import them through `KeychainStore.put` and then delete only those lines. Leave unrelated content in that file.
...
- [ ] Lane 1. Regression lane against trunk. Compare trunk hook (sources the Door 3 auto-sourced env file) with head hook. Trunk lacks Keychain load. Gate that a fresh interactive zsh at head exports a key stored only in Keychain. Save `hook-l1-regression.png`. Pass when head interactive `printenv KEY` is masked and `zsh -c 'printenv KEY'` is empty.
...
- [ ] Lane 8. Import old Door 3 auto-sourced env exports then strip those lines. Save `hook-l8-import.png`. Pass when Keychain has the keys and the stripped lines are gone, and non-zenv lines remain.
...
- [ ] Metric. Interactive zsh startup to first prompt with 20 keys. Trunk sources the Door 3 auto-sourced env file. Head runs `eval "$(zenv env)"`. Also isolate head `zenv env` time.
...
- [ ] Lane 1. Regression lane against trunk. Run trunk `zenv ls` against the Door 3 auto-sourced env file and head `zenv ls` against Keychain. Trunk lacks Keychain `ls`. Gate head `ls` after `put` shows the key masked. Save `cli-l1-ls.png`. Pass when the table contains the key and `********`.
...
- [ ] Lane 4. `set` add then reminder mentions `~/.zshrc`. Save `cli-l4-set.png`. Pass when stdout contains `source ~/.zshrc` and does not mention the Door 3 auto-sourced env file.
...
- [ ] Metric. Time to `zenv ls` with 20 keys. Trunk reads the Door 3 auto-sourced env file. Head lists Keychain attributes without returning secret data. Also isolate head `SecItemCopyMatching` attributes-only call.
...
- [ ] Lane 3. README has no `go install` and no Door 3 auto-sourced env file store claim. Save `retire-l3-readme.png`. Pass when a search for `go install` returns none.
...
- [ ] Lane 8. Book ch01 no longer says Door 3 through the auto-sourced env file. Save `retire-l8-book.png`. Pass when ch01 describes Door 2 `eval "$(zenv env)"`.
...
Write plaintext `export` lines to the Door 3 auto-sourced env file after each Keychain put. Rejected because the operator forbade using that file and that would keep secrets on disk.

Load from the Door 3 auto-sourced env file with `eval "$(zenv env)"` so every shell gets secrets. Rejected because Keychain I/O in non-interactive shells fails silent and slows `zsh -c`. Load stays in `~/.zshrc`.
