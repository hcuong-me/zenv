...
while IFS= read -r line; do
  ...
  ERRORS=$((ERRORS + 1))
done < <(grep -rn 'TODO:\|FIXME:\|PLACEHOLDER\|<INSERT\|TBD' --include='*.md' . \
  --exclude-dir='.agents' --exclude-dir='.build' --exclude-dir='.worktrees' --exclude-dir='thoughts' 2>/dev/null || true)
...
while IFS= read -r line; do
  ...
  --exclude-dir='.agents' --exclude-dir='specs' --exclude-dir='.build' \
  --exclude-dir='.worktrees' --exclude-dir='thoughts' 2>/dev/null | grep -v 'http' || true)
