#!/usr/bin/env bash
# Live check: Keychain load via ~/.zshrc, no ~/.zshenv write.
...
if [[ "$ZSHENV_BEFORE" != "$ZSHENV_AFTER" ]]; then
  echo "FAIL ~/.zshenv was rewritten"
  exit 1
fi
if grep -q LIVECHECK_KEY "$HOME/.zshenv"; then
  echo "FAIL key written to ~/.zshenv"
  exit 1
fi
...
echo "PASS live hook: masked printenv, child sees value, zsh -c empty, zshenv unchanged, env/ls P95 ok"
